...
const HistoryTrips = () => {
  const { cliente: telefono_cliente } = useParams();
  ...
  const fetchTrips = async () => {
    try {
      if (telefono_cliente) {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/geolocation/history-by-client/${telefono_cliente}`);
