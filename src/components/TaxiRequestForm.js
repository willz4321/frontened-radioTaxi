...
const TaxiRequestForm = () => {
  ...
  const handleRequestTaxi = async (e) => {
    ...
    try {
      ...
      console.log(taxiRequestData)
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/geolocation/taxi-request`, taxiRequestData);
      setState(prevState => ({ ...prevState, message: 'Solicitud de taxi enviada exitosamente.' }));
      console.log('Respuesta de la solicitud de taxi:', response.data);
      handleReset(); // Restablecer el formulario después de enviar la solicitud
    } catch (error) {
      setState(prevState => ({ ...prevState, message: 'Error al solicitar el taxi.' }));
  ...
  const getCoordinates = async (address) => {
    ...
    try {
      ...
      // Verificar si results existe y tiene elementos
      ...
      console.error('Error al convertir la dirección en coordenadas:', error.message);
      throw error;
    }
  };
