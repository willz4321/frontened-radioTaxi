...
import { CarFront, BoxSeam, CheckCircle, XCircle, HourglassSplit, ExclamationCircle } from 'react-bootstrap-icons';
import { Line } from 'react-chartjs-2';
import 'chart.js/auto';
import Navbar from './Navbar';

const HistoryAllTrips = () => {
  ...
  const [rejectedTrips, setRejectedTrips] = useState(0);
  const [drivers, setDrivers] = useState([]);
  const tripsPerPage = 10;

  const fetchTrips = async () => {
    try {
      console.log('Consultando historial de todos los viajes');
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/geolocation/history-all`);
  ...
  useEffect(() => {
    applyFilters(trips);
  }, [filter, statusFilter, driverFilter, page]);
  ...
  const applyFilters = (allTrips) => {
    ...
    // Apply driver filter
    if (driverFilter) {
      filtered = filtered.filter(trip => trip.id_taxista === driverFilter);
    }
    ...
    const rejected = filtered.filter(trip => trip.estado === 'rechazado').length;
  ...
  const handleDriverFilterChange = (driverId) => {
    ...
    setPage(1); // Reset pagination when driver filter changes
  };
    ...
    return (
      <Card key={trip.id_viaje} className="mb-3">
        <Card.Body>
          ...
          <Card.Text>Cliente: {trip.telefono_cliente}</Card.Text>
          <Card.Text>Conductor: {trip.nombre_conductor || 'N/A'}</Card.Text>
  ...
  const data = {
    ...
    datasets: [
      ...
      {
        label: 'Viajes Rechazados',
        data: getDataForGraph('rechazado'),
        fill: false,
      ...
      <Container className='contenido'>
        <Row className="text-center mb-2">
          ...
          <Col>
            <Card>
              <Card.Body>
                <ExclamationCircle size={30} color="orange" />
                <Card.Title>{rejectedTrips}</Card.Title>
                <Card.Text>Viajes Rechazados</Card.Text>
              </Card.Body>
            </Card>
          </Col>
        </Row>
          ...
          <Col>
            ...
            <Dropdown onSelect={handleStatusFilterChange}>
              ...
              <Dropdown.Menu>
                ...
                <Dropdown.Item eventKey="rechazado">Rechazados</Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
            <Dropdown onSelect={handleDriverFilterChange}>
              <Dropdown.Toggle variant="black btn-sm" id="dropdown-driver">
                Filtro de Conductor
              </Dropdown.Toggle>
              <Dropdown.Menu>
                <Dropdown.Item eventKey="">Todos</Dropdown.Item>
                {drivers.map(driver => (
                  <Dropdown.Item key={driver.id_usuario} eventKey={driver.id_usuario}>
                    {driver.nombre}
                  </Dropdown.Item>
                ))}
              </Dropdown.Menu>
...
export default HistoryAllTrips;
